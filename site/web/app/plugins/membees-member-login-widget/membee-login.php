...
if (function_exists('json_encode')) {
  ...
  function membee_init() {
    ...
    if ($membee_options['membee_secret']) {
      ...
      if ((!is_user_logged_in())&&($_SERVER['PHP_SELF'] != '/wp-login.php')&&!$_COOKIE['membee-checked']) {         
          setcookie('membee-checked', '1');   
	//Check whether the hosted site is using https.  Ensure this for correct return from the login check
          wp_safe_redirect('https://memberservices.membee.com/feeds/login/LoginCheck.aspx?clientid='.$membee_options['membee_client_id'].'&appid='.$membee_options['membee_app_id'].'&destURL='.urlencode(((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || $_SERVER['SERVER_PORT'] == 443 ? 'https://' : 'http://').$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']));
